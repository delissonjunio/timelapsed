...
| [Monitoring](docs/Monitoring.md) | New Relic APM for all four daemons, off until a key is set |
...
They use the real filesystem and the real `ffmpeg` binary — renders are verified by probing the
output with `ffprobe` — and fake only the NVR.
