...
them on disk as plain JPEGs, and renders hourly, daily and weekly videos with `ffmpeg`. A small
...
**1. Retention must outlast your longest cadence.** A weekly render reads seven days of stills.
If `image_retention_days` is 7 or less, pruning deletes those frames before the render runs and
you silently get no weekly video. Set it to at least 8. Timelapsed prints a warning at startup if
you don't.
